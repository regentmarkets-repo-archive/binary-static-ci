...
public class UpgradeRealAccount_Page {
	...
	{
		WebDriverWait wait = new WebDriverWait(driver,30);
		element = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//a[text()='1']")));
		Assert.assertTrue(element.isDisplayed());
		return element;
	}
	...
	//method to find open account button
		...
		//method to find Account reason element
		public static Select Account_Reason(WebDriver driver)
		{
			Select accountReason=null;
			WebDriverWait wait = new WebDriverWait(driver,30);
			accountReason = new Select(wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("account_opening_reason"))));
